...
definitions necessarily imposed by researchers can inhibit holistic analysis,
may have been inititially motivated by computational limitations [@Logan2017].
...
@Giles-Corti2005 compare gravity-based accessibility scores with an isochrone
specification and show that the former are more predictive of walking behavior;
that is, the attributes of a park matter influence walking more than merely the
distance to it. @Zhang2011 present a population-weighted gravity-based
accessibility to parks metric with a national rather than metropolitan scope, 
though they do not examine its correlation with health outcomes.
...
\end{equation}
the census tract $i$ in miles. The coefficients $\vect{\beta}$ can be
are frequently estimated from household surveys, though in the absence of a survey we
...
researcher to include any attribute of the park as part of its utility
specification. This suggests that not all parks are equal, and that a large park
such as Central Park in Manhattan may provide health and activity benefits over
...
each other. The elements of $W$ are then row-standardized so that each observation
has an equal total influence from all its neighbors.
