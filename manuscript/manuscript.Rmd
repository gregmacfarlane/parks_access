...
abstract: >
  ...
  controlling for alternate explanations. In this paper we apply a comprehensive
  ...
  apply this utility-based accessibility measure to correlate Census tract-level
  ...
  open space data from New York City. Controlling for the
  ...
  activity rates. The data also suggest a negative relationship between park
  access and obesity rates, beyond what is expected through physical activity
  and socioeconomics.
  In doing so, this research contributes a more
...
paper, we apply a holistic and flexible measurement for park accessibility
...
We apply this measurement to study the link between park accessibility and
attractiveness and Census tract-level aggregate physical activity participation
and obesity rates in New York City, controlling for spatially correlated
unobserved effects, spatial spillovers of covariates, and tract-level
...
of a park's attractiveness including its observed social media activity and the
presence of various park amenities. The paper concludes with a discussion of
...
political or statistical boundary [@Mitchell2008; @Stark2014] or within a specific distance
threshold [@Kaczynski2014]. It is also possible to attenuate an isochrone with 
...
merely the distance to it. @Zhang2011 develop a population-weighted,
...
specification where $\exp(X_j\beta + \beta_d d_{ij}) = X_j f(d_{ij})$
...
asserted values [@Handy1997]. @Kaczynski2016 present what is effectively a 
utility-based accessibility score they call "ParkIndex", with the explicit
motivation of developing a uniformly applicable park accessibility statistic,
though they limit this statistic as a way to add heterogeneity within an
isochrone analysis.
...
For each park and 
we calculate the size of the park in acres. Using a Twitter application
...
of park trip destination choice coefficients in New York City, we adopt these
previous estimates.

@Kinnell2006 surveyed park users in New Jersey and applied a multinomial logit
model to determine which factors influenced its perceived utility as a park
...
\end{equation}
with $\lambda_t = 0.99$, $\lambda_c = 0.43$, and $\lambda_p = 0.26$. Kinnell et al.\
did not transform their size and distance estimates, so we retain the previously 
estimated and applied values. We assume that the other covariates in the 
...
physical activity independently of whether his or her neighbors
participate in physical activity. This does not mean that we think it impossible that
the socioeconomic variables of neighboring tracts do not play a role. Mathematically, we assert that the autodependence
relationship on the dependent variable $y = \rho W y + \ldots$ has 
...
texreg(slx_vs_sdem, single.row = TRUE, ci.force = TRUE,  digits = 4,
       ...
       label = "tab:slx-sdem",
       custom.note = "%stars $y = $ tract-level physical activity rate.",
       caption.above = TRUE)
```
...
estimated utility-based coefficient, moving from the least-accessible tract ($A_i = -3$) to the
...
texreg(pa_models[c(2:5)], digits = 4, ci.force = TRUE,
       ...
       custom.model.names = c("Size and Distance", "Twitter", "Amenities", "10-Minute Walk"),
...
As before, the estimates of the other variables are available from the authors.
...
to be between `r 6 * 0.15` percentage points lower and `r 6 * .0053` percentage 
...
texreg(obesity_models[c(2:5)], digits = 4, ci.force = TRUE, 
       booktabs = TRUE, use.packages = FALSE, 
       custom.model.names = c("Size and Distance", "Twitter", "Amenities", "10-Minute Walk"),
       custom.coef.map = list(
...
about. In this paper, we proceed only incrementally towards this by adding
Twitter activity and some selected park amenities as an element of each park's 
attractiveness above and beyond its size. Exploring detailed definitions of
an increased number of amenities on one hand, and market segmentation strategies
on the other, could provide a more nuanced understanding of the relationship
...
logsum, which weights all alternative travel alternatives against each other. 
We leave this as a recommendation for future research.
...
participation rate between `r 6 * .05` and `r 6 * .35` percentage points lower than the most
...
This will in turn enable planners to consider how multiple attributes
of a park --- from its location to its size to its amenities and beyond ---
