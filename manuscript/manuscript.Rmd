...
outcomes has been involved with comparing metropolitan regions against each other. For example,
...
areas within and outside of this threshold. In other applications $D$ might be more
...
For each park, 
we calculate the size of the park in acres. Finally, using a Twitter application
...
parameters for park trips in the Alameda County (Oakland), California by applying
...
York is cost-prohibitive, and the public parks datasets in Alameda County are
...
amenities in Equation \ref{eq:u_multi},
\begin{equation}\label{eq:u_tweets}
V_{ij} = \lambda_s * \log(size_j) + \lambda_d * \log(distance_{ij}) + \lambda_t * trails + \\
  \lambda_{c} * courts + \lambda_p * playgrounds + \lambda_{tw} * YJ(tweets)
\end{equation}
...
Samuels and Neda Madi for helping to assemble the geolocated Twitter data.
