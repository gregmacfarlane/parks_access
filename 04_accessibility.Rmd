...
bg_poly <- st_read("/Users/gregmacfarlane/Library/Caches/tigris/tl_2019_36_bg.shp") %>%
  transmute(geoid = GEOID, p_water = AWATER / (AWATER + ALAND)) %>%
  filter(p_water < 1) %>%
...
distances <- read_all_files(".csv", "data/nyc_shortest_paths_5000m/") %>%
...
check_id = "M010"
check <- bg_poly %>%
  left_join(distances %>% filter(park_id == check_id), by = "geoid") %>%
  mutate(distance = log(distance + 10)) %>% 
  st_transform(4326)
park <- open_spaces %>% st_transform(4326) %>% filter(id == check_id) 

pal <- colorNumeric("plasma", check$distance)
leaflet(check ) %>%
  addProviderTiles(providers$Esri.WorldGrayCanvas) %>%
  addPolygons(color = ~pal(distance) ) %>%
  addPolygons(data = park, color = "green")
```
