...
```{r open_space_points, echo = FALSE}
park_points <- read_csv("data/park_points.csv") %>%
  st_as_sf(coords = c("LONGITUDE", "LATITUDE"), crs = 32618)
```
...
With `r nrow(open_space_points)` and `r nrow(bg_centroids)`, that suggests we
need to compute `r nrow(open_space_points) * nrow(bg_centroids)` shortest
