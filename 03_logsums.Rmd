...
logsum_vars <- tracts %>%
  transmute(
    ...
    access_ls = calculate_park_logsums(
      log(distances) * -1.98, log(open_spaces$acres) * 0.3
    ),
...
tracts_access <- tracts %>%
  ...
  write_rds("data/tracts_acces.rds")
```

Here's a map of the values 
```{r access_map}
...
leaflet(tracts_access) %>%
  ...
  addLayersControl(baseGroups = c("access", "tweets", "10-minute walk"))
```
